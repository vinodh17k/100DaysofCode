...
for j in range(len(sentences)):
    words = nltk.word_tokenize(sentences[j]) # Word Tokenization on sentences list.
    lemmatized_words = [lemmatizer.lemmatize(word) for word in words] #List Comprehension usage and lemmatizing each word of a single sentence at a time.
    sentences[j] = ' '.join(lemmatized_words) # Joining all lemmatized words back into sentences using space delimiter and join function
